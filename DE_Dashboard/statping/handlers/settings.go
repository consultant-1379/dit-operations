...
func commaToService(s []string) (*types.Service, error) {
	...
	var CONFLUENCE_BASE = os.Getenv("CONFLUENCE_BASE")
	var JIRA_BASE = os.Getenv("JIRA_BASE")

	if len(s) != 18 {
		err := fmt.Errorf("does not have the expected amount of %v columns for a service", 17)
		return nil, err
	}
	...
	if strings.Contains(s[17], ":JIRA") {
		supportLink = strings.Replace(JIRA_BASE + s[17], ":JIRA", "", 1)
	} else {
		supportLink = strings.Replace(CONFLUENCE_BASE + s[17], ":CONFLUENCE", "", 1)
	}
	supportLink = strings.Replace(supportLink, "\r", "", 1)
