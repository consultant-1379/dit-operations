...
var User = require('../../modules/users/server/models/user.server.model.js').Schema;
...
async function signInWithUserPassword(req, res) {
  var ldapCalls = [];
  ...
  if (!user) {
    ...
    user.firstName = ldapUser.givenName ? ldapUser.givenName.trim() : 'unknownFN';
    user.lastName = ldapUser.sn ? ldapUser.sn.trim() : 'unknownLN';
    user.username = ldapUser.cn ? ldapUser.cn.trim() : 'unknownUN';
    user.email = ldapUser.mail || ldapUser.displayName.replace(/ /g, '') + '@ericsson.com';
  }
...
exports.signInFromBasicAuthentication = function () {
  return async function (req, res, next) {
    ...
    if (basicAuthenticationUserPass) {
      ...
      try {
        await signInWithUserPassword(req, res);
      } catch (err) {
