#!/bin/bash
# This script is used to restore a DB Backup for the Upgrade Tool.
# Note:
...
docker run -i -v $BACKUP_DIR:/backup --network=$NETWORK armdocker.seli.gic.ericsson.se/dockerhub-ericsson-remote/mongo:4.0.14 mongorestore -u root -p roott --authenticationDatabase admin /backup --host mongodb --drop
